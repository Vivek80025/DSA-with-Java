public class KnapSack {

    //Recursive Approach
    //time complexity:O(2^n)
    //space complexity:O(n)
    public static int knapsackRec(int[] wt,int[] val,int w,int n){
        //basecase
        if(n==0 || w==0){
            return 0;
        }
        if(wt[n-1]<=w){
            return Math.max( (val[n-1]+knapsackRec(wt,val,w-wt[n-1],n-1)) , knapsackRec(wt,val,w,n-1) );
        }
        else{
            return knapsackRec(wt,val,w,n-1);
        }

    }

    //Memoization Approach
    //2 variable change->2d array
    //time complexity:O(n*w)
    //space complexity:O(n*w)
    public static int[][] t; //[n+1][w+1]
    public static int knapsackMemo(int[] wt,int[] val,int w,int n){
        //basecase
        if(n==0 || w==0){
            return 0;
        }

        //memoized
        if(t[n][w] != -1){
            return t[n][w];
        }

        else if(wt[n-1]<=w){
            return t[n][w] = Math.max( (val[n-1]+knapsackMemo(wt,val,w-wt[n-1],n-1)) , knapsackMemo(wt,val,w,n-1) );
        }
        else{
            return t[n][w] = knapsackMemo(wt,val,w,n-1);
        }
    }

    //Tabulation Approach
    //create table t1[n+1][w+1]
    //time complexity:O(n*w)
    //space complexity:O(n*w)
    public static int[][] t1;
    public static int knapsackTab(int[] wt,int[] val,int w,int n){

        //initialization using basecase
        for(int i=0;i<n+1;i++){
            for(int j=0;j<w+1;j++){
                if(i==0 || j==0){
                    t1[i][j] = 0;
                }
            }
        }

        //recursive-->iterative
        for(int i=1;i<n+1;i++){
            for(int j=1;j<w+1;j++){

                if(wt[i-1]<=j){
                    t1[i][j] = Math.max( val[i-1]+t1[i-1][j-wt[i-1]] , t1[i-1][j] );
                }

                else{
                    t1[i][j] = t1[i-1][j];
                }

            }
        }

        return t1[n][w];

    }

    public static void main(String[] args) {
        int[] wt = {1,3,4,5};
        int[] val = {1,4,5,7};
        int w = 7;
        int n = wt.length;

        //for Memoization->initialization of 2d array t
        t = new int[n+1][w+1];
        for(int i=0;i<n+1;i++){
            for(int j=0;j<w+1;j++){
                t[i][j] = -1;
            }
        }

        //for Tabulation
        t1 = new int[n+1][w+1];

        System.out.println("maximum value is: "+knapsackTab(wt,val,w,n));
    }
}
